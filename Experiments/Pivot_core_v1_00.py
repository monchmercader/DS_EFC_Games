#DS EFC Museum Pivot Game
#Authored by Monch Mercader
#This is the code for the EFC Museum Pivot Test Game. This was built using Python
...
def buttonPress(channel):
    # This function gets called every time a button is pressed, if the button pressed is the same as
    # the button that is illuminated, then we set the "correct_button" variable to True.
    global correct_button, button_pressed
...
#REVIEW Check that we have defined the same amount of leds as switches
...
while(True):
    ...
    GPIO.output(led01,True) # Turn on the middle led

    while GPIO.input(switch01) == GPIO.LOW: # Wait until the middle switch has been pressed.
        time.sleep(0.01)
