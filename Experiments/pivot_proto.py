...
# Define our tuples
# LED set defined, Switches set defined
leds = (5,12,17,22,25)
...
def buttonPress(channel):
    ...
    # that is illuminated, then we set the "correct_button" variable to True, otherwise we set the
    # "incorrect_button" variable to True.
...
while(True):
    ...
    loop = 30 # This loop tells us how many buttons are going to be illuminated per game.
    counter = 0 # Create a variable to count how many buttons get illuminated.
    score = 0 # Set our score variable to 0.

    print("Press the lit up button to start")
    GPIO.output(leds[0],True) # Turn on the middle led

    while GPIO.input(switches[0]) == GPIO.LOW: # Wait until the middle switch has been pressed.
        time.sleep(0.01)
    ...
    # Start our countdown
    print("Starting in 5!")
    time.sleep(0.500) # Wait 0.5 seconds
    ...
    while counter < loop:
        ...
        # commenting out and setting own delay with fixed number
        random_number = random.randint(0,max) # Create another random number to be used to turn on one of the leds.

        time.sleep(0.500) # Using real value and Replacing this -> Wait for a random amount of time, as defined above should be random_delay
        ...
        if correct_button:
            points = round(10 - ((time_taken*10)-1),2) # Crude points system. Score between 0 - 10 points.
            # If you take longer than 1 second you score 0. If you take less than 0.1 seconds you score 10.
